...
def run_retrieval(case):
    ...
    for freq in case['frequencies']:
            ...
            for inc in inc_list:
                 ...
                 if site in ['Grassland', 'Shrub']:
                     pol_list = [item for item in pol_list if "HV" not in item and "VH" not in item]
                
                 if site in ['Deciduous','Evergreen']:
                    #  if "VH" in pol_list:
                    #      if "HV" in pol_list:
                    #          pol_list = [item for item in pol_list if item != "VH"]
                    #      else:
                    #          pol_list = ["HV" if item == "VH" else item for item in pol_list]
                   
                    # First, check which HV versions exist
                    hv_versions = set()
                    for item in pol_list:
                         if "HV" in item:
                              hv_versions.add(item.replace("HV", ""))
                    # Process the list
                    new_pol_list = []
                    for item in pol_list:
                         if "VH" in item:
                              base = item.replace("VH", "")
                              if base in hv_versions:
                                   continue  # HV exists → skip (delete VH)
                              else:
                                   item = item.replace("VH", "HV")  # replace VH → HV
                         new_pol_list.append(item)
                    pol_list = new_pol_list
    ...
    if site in ['Grassland', 'Shrub']:
        ...
        # final_dataframes_1 = final_dataframes_[(final_dataframes_['vwc'] > 10)]
        # y = y[(final_dataframes_['vwc'] > 10)]

        # print('final_dataframes_1 forest',final_dataframes_1)
...
#     # num_max_depth = 10
    ...
    model_filename_a = os.path.join(subfolder, model_filename) 
...
#     ################################discrete soil moisture profile########################
#     # # Make predictions
#     # y_g_pred_a = model_a.predict(X_g[freq_pol])
#     # # Evaluate the model
#     # rmse_g_a = root_mean_squared_error(y_g['SMVWC_10_CM'], y_g_pred_a)
#     # # Make predictions
#     # y_g_pred_b = model_b.predict(X_g[freq_pol])
#     # # Evaluate the model
#     # rmse_g_b = root_mean_squared_error(y_g['SMVWC_20_CM'], y_g_pred_b)
#     # # Make predictions
#     # y_g_pred_c = model_c.predict(X_g[freq_pol])
#     # # Evaluate the model
#     # rmse_g_c = root_mean_squared_error(y_g['SMVWC_50_CM'], y_g_pred_c)
#     #  # Make predictions
#     # y_g_pred_vwc = model_vwc.predict(X_g[freq_pol])
#     # # Evaluate the model
#     # rmse_g_vwc = root_mean_squared_error(y_g['VWC'], y_g_pred_vwc)
#     # # print(rmse_c,rmse_b,rmse_a,rmse_vwc)

#     # return rmse_g_a,rmse_g_b,rmse_g_c,rmse_g_vwc
...
# ##########################second order poly soil moisture profile##################################
    ...
    rmse_sm = root_mean_squared_error(np.array([X_test_screen['sm0'],X_test_screen['sm20'],X_test_screen['sm50']]).flatten(), 
                                     np.array([X_test_screen['sm0_re'],X_test_screen['sm20_re'],X_test_screen['sm50_re']]).flatten())
